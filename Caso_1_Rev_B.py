...
def run_simulation(L, r, n, m, Cp, rho, Ti, T0, q_fluxo, t_final, dt):
    ...
    t_teste = dt
    T_permanente_anterior = np.inf
    T_permanente_atual = np.zeros(n)

    # Loop para testar diferentes tempos até que o regime permanente seja alcançado
    while np.max(np.abs(T_permanente_atual - T_permanente_anterior)) > 0.01:  # Critério de convergência
        T_permanente_anterior = T_permanente_atual.copy()
        
        # Resolvendo a EDO usando o odeint para o regime permanente
        T_permanente = odeint(dTdt_function, T, [0, t_teste])[-1]
        T_permanente_atual = T_permanente
